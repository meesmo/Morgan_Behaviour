...
class data():
    ...
    def __init__(self, protocol_name, protocol_description, n_trials,
        ...
        *All parameters are stored as attributes with the same name.*
        ...
        self.trace =  np.empty(self.n_trials) * np.nan
    ...
    def Store(self):
        ...
        with h5py.File(self.filename, 'w') as f:
            ...
            self.opto_start = f.create_dataset('opto_start', data=self.opto_start)
            self.opto_end = f.create_dataset('opto_end', data=self.opto_end)
            self.trace = f.create_dataset('trace', data=self.trace)
            ...
            trace.attrs['title'] = 'time stamp of trace period'
